...
public class LocationJabberer extends Thread {
	...
	public LocationJabberer(String host, String port, String service, String user, 
			String password, String ressource, String androidbuddy, String interval) {
		this.host = host;
		this.port = port;
	...
	private void xmppTalk() {
		...
		int interval = 1000; 
		try { interval = Integer.parseInt(this.interval); } catch (NumberFormatException e) {}
		for (Coordinate c: this.coordinates) {
			...
			try { LocationJabberer.sleep(interval); } catch (InterruptedException e) {}
		}
	...
	private void xmppRosterPost(XMPPConnection connection) {
		...
		blIQ.setIdentity(connection.getUser());
