...
public class OutgoingTransfer extends Transfer {
	...
	protected boolean onReady() {
		...
		try {
			if (!XMPPUtil.isFullyQualified(this.xmppFile.to))
				xmppService.getServiceDiscoveryService().discoverItem(
						xmppMessenger, xmppMessenger, this.id, this.xmppFile.to, null);
			else
				this.onResourceDiscovered(null);
	...
	protected void onResourceDiscovered(String fullyQualifiedJid) {
		...
		final Messenger xmppMessenger  = this.manager.getService().getXmppMessenger();
		try {
			if (fullyQualifiedJid != null)
				this.xmppFile.to = fullyQualifiedJid;
	...
	public void handleMessage(Message msg) {
		if (msg.what == ConstMXA.MSG_DISCOVER_ITEMS
			...
			List<Parcelable> pp = (ArrayList<Parcelable>)
					msg.getData().getParcelableArrayList("DISCOVER_ITEMS");
			if (pp.size() > 0)
				this.onResourceDiscovered(
						((DiscoverItem)pp.get(0)).jid
					);
			else
				this.onError(-104, "Couldn't find out the roster items ressource.");
