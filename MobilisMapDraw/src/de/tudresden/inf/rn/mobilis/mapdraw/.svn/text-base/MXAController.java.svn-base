...
import de.tudresden.inf.rn.mobilis.saaas.IXMPPIQCallback;
...
public class MXAController {
	...
	public void loginToXMPP() {
		...
		try {
			mXMPPService.connect(mStdAckMessenger, readXMPPPreferences());
	...
	private Handler xmppResultHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_CONNECT:
				...
				String status;
				if (msg.arg1 == MSG_STATUS_SUCCESS) status = "XMPP is connected";
	...
	private IXMPPIQCallback mIQCallback = new IXMPPIQCallback.Stub() {
		...
		public void processIQ(XMPPIQ iq) throws RemoteException {
			Message msg = Message.obtain(xmppIQHandler, MSG_IQ_RECEIVED);
			Bundle data = new Bundle();
